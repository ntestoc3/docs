...
(defn my-form
  "加载form"
  []
  (let [form (identify (DepthWindow.))]

    ;; 更新quote-coin的model
    (gui/config! (gui/select form [:#base-coin]) :model base-coins)
    (update-quote-coin-model! form (-> (:base-coin @coin-pair-data)
                                        get-quote-coins))

    ;; 先绑定事件，再设置默认值
    (add-behaviors form)
    (gui/value! form @coin-pair-data)
